...
pub fn likelihood(odds: Vec<f64>) -> f64 {
    return odds[0] / (odds[0] + odds[1]);
}

pub fn recalculate(prior: Vec<f64>, likelihoods: Vec<Vec<Vec<f64>>>) -> Vec<f64> {
    prior.into_iter().enumerate().map(|(hyp_idx, prior_val)| {
        likelihoods.iter().fold(prior_val, |acc, ev| acc * likelihood(ev[hyp_idx].clone()))
    }).collect()
}

pub fn recalculate_to(prior: Vec<f64>, likelihoods: Vec<Vec<Vec<f64>>>, to: usize) -> Vec<f64> {
    prior.into_iter().enumerate().map(|(hyp_idx, prior_val)| {
        likelihoods.iter().take(to).fold(prior_val, |acc, ev| acc * likelihood(ev[hyp_idx].clone()))
    }).collect()
}

fn save_data(data: &BayesData) {
    let serialized = serde_json::to_string(&data).unwrap();
    SessionStorage::set("bayes_component".to_string(), &serialized).unwrap();
}
...
impl Component for BayesComponent {
    ...
    fn create(_ctx: &yew::Context<Self>) -> Self {
        ...
        match SessionStorage::get::<String>("bayes_component") {
            Ok(serialized) => {
    ...
    fn view(&self, ctx: &yew::Context<Self>) -> Html {
        ...
        let onchange_prior = ctx.link().callback(move |(u, e, h): (usize, Vec<f64>, Vec<AttrValue>)| Msg::Prior(u, e, h));
        let onchange_posterior = ctx.link().callback(move |(_, _, _): (usize, Vec<f64>, Vec<AttrValue>)| Msg::Posterior);
        ...
        let onchange_evidence = |ev_idx: usize| {
            ctx.link()
                .callback(move |evidence_msg: EvidenceCallback| {
                    match evidence_msg {
                        EvidenceCallback::OddsUpdate(hyp_idx, new_odds) =>  Msg::Evidence(ev_idx, hyp_idx, new_odds),
        ...
        let hypotheses: _ = self.data.hypotheses.clone().into_iter().map(|h| AttrValue::from(h)).collect::<Vec<AttrValue>>();
    ...
    fn update(&mut self, ctx: &yew::Context<Self>, msg: Self::Message) -> bool {
        match msg {
            ...
            Msg::AddEvidence => {
                ...
                self.data.likelihoods.push(vec![vec![1.0, 1.0]; self.data.hypotheses.len()]);
            }
        ...
        self.data.posterior_odds = recalculate(self.data.prior_odds.clone(), self.data.likelihoods.clone());
